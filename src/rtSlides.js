...
function rtSlideNav(slidesContainer, wrapper, width, max) {
    ...
    nav.addEventListener('click', (e) => {
        if (e.target.accessKey === 'right' && xPos < max) { xPos += width }
        if (e.target.accessKey === 'left' && xPos >= 0) { xPos -= width }
...
function keyListener(wrapper, width, max) {
    document.addEventListener('keydown', ({ key }) => {
        if (key === 'ArrowRight' && xPos < max) { xPos += width }
        if (key === 'ArrowLeft' && xPos >= 0) { xPos -= width }
        wrapper.style.transform = `translateX(-${xPos}px)`
    })
}
